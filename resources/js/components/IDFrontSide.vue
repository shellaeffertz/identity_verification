...
import FrontSideIdInformationModal from './FrontSideIdInformationModal.vue';
...
const captureAndRecord = async () => {
  ...
  setTimeout(async () => {
    ...
    form.post(`/store-front-side/${props.uuid}`);
...
<template>
  <div class="fixed inset-0 bg-black overflow-hidden">
    <p class="absolute top-1 left-1/2 transform -translate-x-1/2 bg-black/70 text-white px-6 py-3 rounded-md z-50 text-center w-full">
      Please capture the front side of your ID card by holding it steady and ensuring it fits entirely within the centered frame on your screen.
    </p>
    ...
    >
      <span v-if="!isUploading && !isRecording">Capture Front ID</span>
      <LoadingSpinner v-if="isUploading || isRecording" />
    </button>

  </div>

  <FrontSideIdInformationModal />
